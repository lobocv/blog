...
Once we have our initial scaffolding setup, we can start writing tests for our code flows. I always start off testing the
...
// This would be a model used by the application, but I am redefining it here for clarity of reading the tests
type Post struct {
	ID string
	Type string
...
func (s *LinkedinCollectorTestSuite) SetupTest() {
	...
	s.fix = Fixtures{
	    ...
	    videoPost: &Post{
		                ID: "2",
						Type: "VIDEO",
						CreatedDate: time.Now().Add(-7*24*time.Hour),
...
func (s *LinkedinCollectorTestSuite) setupListUGCCall(succeed bool) {
	call := s.mocks.LinkedinClient.On("ListUGC", mock.Anything, s.fix.linkedinPageID)
	if succeed {
...
func (s *LinkedinCollectorTestSuite) setupEntityShareStatisticsCall(succeed bool, mockPost *Post) {
	call := s.mocks.LinkedinClient.On("EntityShareStatistics", mock.Anything, mockPost.ID)
	if succeed {
...
We have an edge case in that the `EntityShareStatistics` endpoint only lets you gather metrics for posts that are less 
...
becomes more useful when your mock setup calls start having too many parameters and thus start doing too many things in one place.
...
lots of edge cases based solely on varying inputs. They are not great for testing complex workflows.
...
where test cases and expectations are written in a natural language, such as english. It allows for business people
to write test cases and developers implement them. I am a huge fan of Behavior Driven Testing as it leads to higher 
quality tests and a better understanding between both development and business teams.
...
These two libraries have a unique test execution strategy. Tests are written in nested structure which end up creating
nodes of a graph. Tests are run by traversing this graph in depth-first
